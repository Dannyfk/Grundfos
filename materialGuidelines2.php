<html>
  <head>
    <meta charset="utf-8">
    <title>MyGrundfos Marketing Content - Material Guidelines</title>
    <link rel="stylesheet" href="css/style.css">
    <link rel="stylesheet" href="css/Guideline.css">
    <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.2.1/jquery.min.js"></script>
  </head>
  <body>
    <img src="images/header.png" alt="header" id="header-grundfos">
    <section id="wrapper-content">
    <?php include 'aside.php'; ?>
    <section id="content" >






<div class="wrapper">
<h1> Material Guidelines </h1>


<section class="gridguide">
    <div class="fade">

        <h2> Store </h2>
        <img src="images/store.png" data-id="storeselect">
        <p> Hvordan i kan anvende vores materiale i jeres butik   </p>

    </div>

    <div class="fade">

        <h2> Social Media </h2>
        <img src="images/social.png" data-id="socialselect">
        <p> Hvordan i kan markedsføre på de sociale medier </p>

    </div>

    <div class="fade">

        <h2> Webshop </h2>
        <img src="images/webshop.png" data-id="webshopselect">
        <p> Til jer med en webshop, der vil sælge online </p>

    </div>

</section>



<section class="selection">

         <h2> Store </h2>
         <h2> Social Media </h2>
         <h2> Webshop </h2>
         <img data-id="storeselect" src="images/store.png">
         <img data-id="socialselect" src="images/social.png">
         <img data-id="webshopselect" src="images/webshop.png">


</section>

<!-- <<<<<<<STORE>>>>>>>> -->

<section class="guides" id="storeselect">

 <input id="toggle1" type="checkbox" unchecked>
  <label for="toggle1"> Images</label><span src="images/materialtypes/imagesicon.png"></span>

    <div class="images1">

        <p> Lorem dolor sit amet, consectetur adipiscing elit. Etiam tortor leo, viverra et euismod ut, vestibulum nec odio. Nulla facilisi. Vivamus in ex pretium tortor eleifend vulputate. Maecenas et venenatis nisi. Nulla ultrices vitae lorem nec euismod. Fusce scelerisque velit libero, id porta velit ultricies vitae. </p>
        <img src="images/store.png">
        <img src="images/social.png">
        <p> Lorem ipsum dolor sit amet, consectetur adipiscing elit. Etiam tortor leo, viverra et euismod ut, vestibulum nec odio. Nulla facilisi. Vivamus in ex pretium tortor eleifend vulputate. Maecenas et venenatis nisi. Nulla ultrices vitae lorem nec euismod. Fusce scelerisque velit libero, id porta velit ultricies vitae. </p>
        <img src="images/store.png">
        <img src="images/social.png">

    </div>

<input id="toggle2" type="checkbox" unchecked>
  <label for="toggle2">Selling Points</label>

    <div class="video1">

        <p> Lorem ipsum dolor sit amet, consectetur adipiscing elit. Etiam tortor leo, viverra et euismod ut, vestibulum nec odio. Nulla facilisi. Vivamus in ex pretium tortor eleifend vulputate. Maecenas et venenatis nisi. Nulla ultrices vitae lorem nec euismod. Fusce scelerisque velit libero, id porta velit ultricies vitae. </p>
        <img src="images/store.png">
        <img src="images/social.png">
        <p> Lorem ipsum dolor sit amet, consectetur adipiscing elit. Etiam tortor leo, viverra et euismod ut, vestibulum nec odio. Nulla facilisi. Vivamus in ex pretium tortor eleifend vulputate. Maecenas et venenatis nisi. Nulla ultrices vitae lorem nec euismod. Fusce scelerisque velit libero, id porta velit ultricies vitae. </p>
        <img src="images/store.png">
        <img src="images/social.png">

    </div>

<input id="toggle3" type="checkbox" unchecked>
  <label for="toggle3">Banners</label>

    <div class="banners1">

        <p> Lorem ipsum dolor sit amet, consectetur adipiscing elit. Etiam tortor leo, viverra et euismod ut, vestibulum nec odio. Nulla facilisi. Vivamus in ex pretium tortor eleifend vulputate. Maecenas et venenatis nisi. Nulla ultrices vitae lorem nec euismod. Fusce scelerisque velit libero, id porta velit ultricies vitae. </p>
        <img src="images/store.png">
        <img src="images/social.png">
        <p> Lorem ipsum dolor sit amet, consectetur adipiscing elit. Etiam tortor leo, viverra et euismod ut, vestibulum nec odio. Nulla facilisi. Vivamus in ex pretium tortor eleifend vulputate. Maecenas et venenatis nisi. Nulla ultrices vitae lorem nec euismod. Fusce scelerisque velit libero, id porta velit ultricies vitae. </p>
        <img src="images/store.png">
        <img src="images/social.png">

    </div>




</section>

<!-- <<<<<Social>>>>> -->

<section class="guides" id="socialselect">

 <input id="toggle4" type="checkbox" unchecked>
  <label for="toggle4">Images</label>

    <div class="images2">

        <p> Lorem ipsum dolor sit amet, consectetur adipiscing elit. Etiam tortor leo, viverra et euismod ut, vestibulum nec odio. Nulla facilisi. Vivamus in ex pretium tortor eleifend vulputate. Maecenas et venenatis nisi. Nulla ultrices vitae lorem nec euismod. Fusce scelerisque velit libero, id porta velit ultricies vitae. </p>
        <img src="images/store.png">
        <img src="images/social.png">
        <p> Lorem ipsum dolor sit amet, consectetur adipiscing elit. Etiam tortor leo, viverra et euismod ut, vestibulum nec odio. Nulla facilisi. Vivamus in ex pretium tortor eleifend vulputate. Maecenas et venenatis nisi. Nulla ultrices vitae lorem nec euismod. Fusce scelerisque velit libero, id porta velit ultricies vitae. </p>
        <img src="images/store.png">
        <img src="images/social.png">

    </div>

<input id="toggle5" type="checkbox" unchecked>
  <label for="toggle5">Videos</label>

    <div class="video2">

        <p> Lorem ipsum dolor sit amet, consectetur adipiscing elit. Etiam tortor leo, viverra et euismod ut, vestibulum nec odio. Nulla facilisi. Vivamus in ex pretium tortor eleifend vulputate. Maecenas et venenatis nisi. Nulla ultrices vitae lorem nec euismod. Fusce scelerisque velit libero, id porta velit ultricies vitae. </p>
        <img src="images/store.png">
        <img src="images/social.png">
        <p> Lorem ipsum dolor sit amet, consectetur adipiscing elit. Etiam tortor leo, viverra et euismod ut, vestibulum nec odio. Nulla facilisi. Vivamus in ex pretium tortor eleifend vulputate. Maecenas et venenatis nisi. Nulla ultrices vitae lorem nec euismod. Fusce scelerisque velit libero, id porta velit ultricies vitae. </p>
        <img src="images/store.png">
        <img src="images/social.png">

    </div>

    <input id="toggle6" type="checkbox" unchecked>
  <label for="toggle6">Banners</label>

    <div class="banners2">

        <p> Lorem ipsum dolor sit amet, consectetur adipiscing elit. Etiam tortor leo, viverra et euismod ut, vestibulum nec odio. Nulla facilisi. Vivamus in ex pretium tortor eleifend vulputate. Maecenas et venenatis nisi. Nulla ultrices vitae lorem nec euismod. Fusce scelerisque velit libero, id porta velit ultricies vitae. </p>
        <img src="images/store.png">
        <img src="images/social.png">
        <p> Lorem ipsum dolor sit amet, consectetur adipiscing elit. Etiam tortor leo, viverra et euismod ut, vestibulum nec odio. Nulla facilisi. Vivamus in ex pretium tortor eleifend vulputate. Maecenas et venenatis nisi. Nulla ultrices vitae lorem nec euismod. Fusce scelerisque velit libero, id porta velit ultricies vitae. </p>
        <img src="images/store.png">
        <img src="images/social.png">

    </div>




</section>

<!-- <<<<<Webshop>>>>>  -->

<section class="guides" id="webshopselect">

 <input id="toggle7" type="checkbox" unchecked>
  <label for="toggle7">Images</label>

    <div class="images3">

        <p> Lorem ipsum dolor sit amet, consectetur adipiscing elit. Etiam tortor leo, viverra et euismod ut, vestibulum nec odio. Nulla facilisi. Vivamus in ex pretium tortor eleifend vulputate. Maecenas et venenatis nisi. Nulla ultrices vitae lorem nec euismod. Fusce scelerisque velit libero, id porta velit ultricies vitae. </p>
        <img src="images/store.png">
        <img src="images/social.png">
        <p> Lorem ipsum dolor sit amet, consectetur adipiscing elit. Etiam tortor leo, viverra et euismod ut, vestibulum nec odio. Nulla facilisi. Vivamus in ex pretium tortor eleifend vulputate. Maecenas et venenatis nisi. Nulla ultrices vitae lorem nec euismod. Fusce scelerisque velit libero, id porta velit ultricies vitae. </p>
        <img src="images/store.png">
        <img src="images/social.png">

    </div>

<input id="toggle8" type="checkbox" unchecked>
  <label for="toggle8">Videos</label>

    <div class="video3">

        <p> Lorem ipsum dolor sit amet, consectetur adipiscing elit. Etiam tortor leo, viverra et euismod ut, vestibulum nec odio. Nulla facilisi. Vivamus in ex pretium tortor eleifend vulputate. Maecenas et venenatis nisi. Nulla ultrices vitae lorem nec euismod. Fusce scelerisque velit libero, id porta velit ultricies vitae. </p>
        <img src="images/store.png">
        <img src="images/social.png">
        <p> Lorem ipsum dolor sit amet, consectetur adipiscing elit. Etiam tortor leo, viverra et euismod ut, vestibulum nec odio. Nulla facilisi. Vivamus in ex pretium tortor eleifend vulputate. Maecenas et venenatis nisi. Nulla ultrices vitae lorem nec euismod. Fusce scelerisque velit libero, id porta velit ultricies vitae. </p>
        <img src="images/store.png">
        <img src="images/social.png">

    </div>

    <input id="toggle9" type="checkbox" unchecked>
  <label for="toggle9">Banners</label>

    <div class="banners3">

        <p> Lorem ipsum dolor sit amet, consectetur adipiscing elit. Etiam tortor leo, viverra et euismod ut, vestibulum nec odio. Nulla facilisi. Vivamus in ex pretium tortor eleifend vulputate. Maecenas et venenatis nisi. Nulla ultrices vitae lorem nec euismod. Fusce scelerisque velit libero, id porta velit ultricies vitae. </p>
        <img src="images/store.png">
        <img src="images/social.png">
        <p> Lorem ipsum dolor sit amet, consectetur adipiscing elit. Etiam tortor leo, viverra et euismod ut, vestibulum nec odio. Nulla facilisi. Vivamus in ex pretium tortor eleifend vulputate. Maecenas et venenatis nisi. Nulla ultrices vitae lorem nec euismod. Fusce scelerisque velit libero, id porta velit ultricies vitae. </p>
        <img src="images/store.png">
        <img src="images/social.png">

    </div>




</section>

</div>
<script src="js/jquery-3.2.1.min.js"></script>
<script src="js/guidelines.js"> </script>









      </section>
    </section>
    </section>
    <?php include 'footer.php'; ?>
  </body>
</html>
